    ...
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-8 sm:p-10 space-y-6">
            <div class="text-center">
                <h2 class="text-3xl font-extrabold text-slate-900">เขียนบทความใหม่</h2>
                <p class="text-slate-500 mt-1.5">เริ่มต้นเขียนไอเดียดีๆ และแบ่งปันให้ทุกคนได้อ่านกัน</p>
            </div>

            <form method="POST" action="/insert" class="space-y-6">
                @csrf
                <div class="space-y-1.5">
                    <label for="title" class="block text-sm font-semibold text-slate-700">ชื่อบทความ</label>
                    <input type="text" name="title" id="title" placeholder="พิมพ์ชื่อบทความของคุณที่นี่..." class="input input-bordered w-full border-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-xl @error('title') border-red-500 @enderror">
                    @error('title')
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea name="content" id="content" rows="6" class="textarea textarea-bordered w-full border-slate-200 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-xl @error('content') border-red-500 @enderror"></textarea>
                    @error('content')
                        <div class="text-red-500 text-xs mt-1 flex items-center gap-1">
                            <span>⚠️</span> {{ $message }}
                        </div>
